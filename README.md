...
| `confluence` | 多指标共振 | EMA+SuperTrend+MACD+RSI+布林+量价 | 加权投票打分，ADX当闸，EMA200做趋势过滤 |

**为什么共振策略不是"10个指标全部 AND"？** 把 10 个指标当成"必须同时满足"，
结果是几乎不开仓且严重过拟合。`confluence` 用**加权投票打分**（score∈[-1,1]）：
每个指标投一票，得分过线才开仓；ADX 当总闸（无趋势不做），EMA200 当大趋势过滤器
（不在下跌趋势里追多）。实测样本外验证是**绿灯**（比单指标策略更抗过拟合）——
多指标互相印证减少假信号，这才是"多指标合一"的正确打开方式。面板扫描页会显示
每个标的的**共振分**，让你直观看到多空合力强弱。
